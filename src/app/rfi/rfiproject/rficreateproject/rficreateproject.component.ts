...
export class RFICREATEPROJECTComponent {
  ...
  async ngOnInit() {
    this.deviceInfo = this.deviceService.getDeviceInfo();
     ...
     this.passID = {
      ...
      // let _rfiNumber = await this.getFBCounterVariation();
  ...
  getProject(){
    ...
    // this.data_api.getFBProject(this.passID.id).pipe(first()).subscribe(data => 
        ...
        //     this.rfiForm.patchValue({
        //       rfiNumber: displayJobNum + varSetCode + this.checkDigitNumbers( parseInt(this.projectData.counterVariation) + 1 )
        //     });
        // }else{
        //     this.rfiForm.patchValue({
...
computeGroupTotal(groupIndex){
  ...
  onToppingRemoved(topping, index: number) {
    ...
    public findInvalidControls() {
      const invalid = [];
      const controlsVariation = this.rfiForm.controls
...
if(str){
  ...
  saveStepCheckValidation(){
      ...
      if(  (this.rfiForm.value.projectOwner.length < 1) &&  (this.setProjectVariationRecipient.length < 1) ){
      ...
      if(tobeuploadPDFs > 0){
        console.log('saveStepUploadQuotes going')
        this.saveStepUploadQuotes();
      }else{
        console.log('saveStepUploadPDF going')
  ...
  async saveStepCreateVariation(){
      ...
      this.data_api.updateFBCounterProjectVariation(this.rfiForm.value.projectId, this.displayVariationNumber ).then(() => {
           console.log('Variation Counter incremented!');
      });
      ...
      this.data_api.createFBRFI(this.variationId,this.rfiForm.value).then(data => {
        ...
        this.router.navigate(['/rfi/project/'+this.passID.id+'/edit/'+this.variationId]);  
...
btnFunctionList(){
  console.log('dirty', this.rfiForm.dirty);
...
public findInvalidControls() {
    ...
    console.log(controlsVariation, 'controlsVariation')
