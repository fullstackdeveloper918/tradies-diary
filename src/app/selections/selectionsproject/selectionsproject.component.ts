...
import { SelectionsProjectRenderComponent } from './selectionsproject-render.component';
...
export class SelectionsProjectComponent implements OnInit {
    ...
    // SEARCH BY NAME
    searchByNameFilterSelection(event: any) {
      ...
      if (event.target.value.length > 0) {
          query = this.afs.collection('/accounts').doc(this.accountFirebase)
              .collection('/selections', ref => ref
                  .where("projectId", '==', this.passID.id)
                  .where("selectionName", "==", event.target.value)
        ...
        query = this.afs.collection('/accounts').doc(this.accountFirebase)
            .collection('/selections', ref => ref
                .where("projectId", '==', this.passID.id)
                .orderBy("selectionNumber", 'desc')  // For ordering variations by number or any other criteria
                .limit(10)
            );
    }
